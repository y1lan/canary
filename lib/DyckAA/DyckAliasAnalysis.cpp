...
void DyckAliasAnalysis::printAliasSetInformation() {
  ...
  //    outs() << "   " << NoAliasNum << " no alias responses (" << (unsigned
  //    long)PercentOfNoAlias << "%)\n\n";
  //}
  ...
  ///*if (DotAliasSet) */
  {
    ...
    while (RepIt != Reps.end()) {
      ...
      fprintf(AliasRel, "a%d[color=\"%s\"label=\"%s\"];\n", Idx,
              LabelColor.c_str(), LabelDesc.c_str());
    ...
    while (RepIt != Reps.end()) {
      ...
      std::map<DyckGraphEdgeLabel *, std::set<DyckGraphNode *>> &OutVs =
          DGN->getOutVertices();
